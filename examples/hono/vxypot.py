...
R1 = np.linspace(R1MIN, R1MAX, num=5)
R2 = np.linspace(R2MIN, R2MAX, num=5)
T2 = np.linspace(T2MIN, T2MAX, num=5)
R3 = np.linspace(R3MIN, R3MAX, num=5)
T1 = np.linspace(T1MIN, T1MAX, num=5)
PH = np.linspace(PHMIN, PHMAX, num=5)
...
while RESER >= TOL:
    ...
    PARAMS_OPT_CHEB = mini(rho, CARRAY,
                           method='Powell', options={'maxfev': 5000})
